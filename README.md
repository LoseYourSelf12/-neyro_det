...
- `drow_zones.py` – графический редактор масок зон для видео и изображений.
...
Внимательно изучите и установите согласно инструкции внутри! Лучше устанавливать постепенно.

### Запуск на Jetson Nano

На устройстве Jetson Nano должны быть все необходимые версии библиотек. Далее сервис запускается командой, предварительно изменив конфигурацию:
...
Файл `demo.py` реализует GUI‐демонстрацию на PyQt6 с визуализацией зон и статистики по трём камерам. Для редактирования масок зон можно использовать `drow_zones.py`, который поддерживает загрузку видеофайлов и отдельных изображений.
